...
const mostrarModal = (modelo = MODELO_BASE) => {
    ...
    if (modelo.idCategoria == 0) {
        ...
        .then(responseJson => {
            if (responseJson.estado) {
                ...
                swal('Oops', responseJson.mensaje, 'error');
            }
        })
    } else {
        ...
        .then(responseJson => {
            if (responseJson.estado) {
                ...
                $('#modalData').modal('hide');
                swal('Listo!', 'La categoria ha sido actualizada', 'success');
            } else {
                swal('Oops', responseJson.mensaje, 'error');
            }
        });
...
$("#tbdata tbody").on('click', '.btn-editar', function () {
    $(this).closest('tr').hasClass('child') ? filaSeleccionada = $(this).closest('tr').prev() : filaSeleccionada = $(this).closest('tr');
...
$("#tbdata tbody").on('click', '.btn-eliminar', function () {
    let fila;
    $(this).closest('tr').hasClass('child') ? fila = $(this).closest('tr').prev() : fila = $(this).closest('tr');
            ...
            if (respuesta) {
                ...
                .then(responseJson => {
                    if (responseJson.estado) {
                        ...
                        swal('Listo!', 'La cateogira ha sido eliminada', 'success');
                    } else {
                        swal('Oops', responseJson.mensaje, 'error');
                    }
                })
